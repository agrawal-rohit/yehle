...
- Use `kebab-case` for file names, with optional file type suffixes - `user-profile.ts`, `user-profile.utils.ts`, `user-profile.hook.ts`, `user-profile.api.ts`, etc.
...
- Prefer `as const` assertions and `satisfies` to help the compiler infer accurate types.