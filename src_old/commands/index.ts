...
function parseListCliOptions(
	...
	filters: Partial<
		Record<Exclude<RegistryFacetKey, "type" | "tag">, string>
	>;
} {
	...
	const tagOption = options.tag;
	const tag = typeof tagOption === "string" ? tagOption : undefined;
	...
	return { all, type, tag, values, filters };
}
...
/**
 * Register create, add, and list CLI commands against the loaded registry.
	...
	listCmd
		...
		.action(async (options: Record<string, unknown>) => {
			try {
				...
				const { all, type, tag, values, filters } = parseListCliOptions(
					options,
					facets,
				);
