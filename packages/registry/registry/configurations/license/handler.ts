...
/** Prompt for an SPDX license and write a filled LICENSE file. */
const handler: ItemHandler = {
	...
	async files(ctx) {
		const { licenseText } = ctx.variables;
		if (!licenseText)
			throw new Error("License handler is missing license text.");
		return [{ target: "LICENSE", content: licenseText }];
	},
};
